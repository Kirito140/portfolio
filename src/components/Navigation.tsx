...
function Navigation({ parentToChild }: any) {
  const { mode: initialMode } = parentToChild;

  const [mobileOpen, setMobileOpen] = useState<boolean>(false);
  const [scrolled, setScrolled] = useState<boolean>(false);
  const [mode, setMode] = useState<string>(initialMode || "dark");

  // Gérer le toggle dark/light
  const modeChange = () => {
  ...
  // Initialiser le mode depuis localStorage
  useEffect(() => {
